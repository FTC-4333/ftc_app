package org.firstinspires.ftc.teamcode;

public class UniversalConstants

{
    static final String LEFT1NAME      = "l1";
    static final String LEFT2NAME      = "l2";
    static final String RIGHT1NAME     = "r1";
    static final String RIGHT2NAME     = "r2";
  //  static final String LEFTCOLLECTOR  = "lc";
  //  static final String RIGHTCOLLECTOR = "rc";
    //arm0 is the bottom part of the arm, the "shoulder"
    //arm1 is the top part, the "forearm"
   // static final String ARM0           = "a0";
   // static final String ARM1           = "a1";
    static final String LIFTER         = "l";
}